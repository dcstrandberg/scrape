...
def update_scrape_db(df, replace = False):
    ...
    for aDate in dateList:
        ...
        result = execute_query(conn, "SELECT COUNT(Date) FROM scrape_data WHERE Date='" + str(aDate) + "'", "Error in Before Count")
        ...
        if replace:
            execute_query(conn, "DELETE FROM scrape_data WHERE Date='" + str(aDate) + "'", "Error in Deletion")
            result = execute_query(conn, "SELECT COUNT(Date) FROM scrape_data WHERE Date='" + str(aDate) + "'", "Error in PostDelete Count")
            temp = result.fetchone()
    ...
    for aDate in dateList:
        ...
        result = execute_query(conn, "SELECT COUNT(Date) FROM scrape_data WHERE Date='" + str(aDate) + "'", "Error in New Entry Count")
        temp = result.fetchone()
